...
The reward model gates *pre-launch*; the live experiment confirms. Two things govern this stage. **Rotation against wear-out** — a fresh headline decays (half-life of 60–120 days at stable volume), so generation runs continuously and the reward model keeps swapping the champion, flattening the decay rather than resetting it on a quarterly cadence. **The bandit boundary** — in a small, well-understood variant space (three subject-line styles on a transactional email) the whole pipeline is overkill; a multi-armed bandit converges faster and more reliably. The generate→check→judge→select→rotate machine pays off only when the variant space is too large to explore by bandit.
