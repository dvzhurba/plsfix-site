...
Generation was never the bottleneck. A copy team could always write more headlines than could be tested; an LLM makes the imbalance absurd — a thousand variants a minute, most of them correct, on-brand and interchangeable, a few actively dangerous. The hard problem is not producing copy; it is everything between the model's output and a real impression on a real user.
...
The reward model gates *pre-launch*; the live experiment confirms. Two things govern this stage. **Rotation against wear-out** — a fresh headline decays (half-life of 60–120 days at stable volume), so generation runs continuously and the reward model keeps swapping the champion, flattening the decay rather than resetting it on a quarterly cadence. The decay is neither hypothetical nor cheap: one reused headline pair on a live campaign fell from roughly 6.0% to 4.2% open rate over about three months at flat volume — a third of its performance gone, quietly costing tens of thousands of opens a fresh variant would have kept. **The bandit boundary** — in a small, well-understood variant space (three subject-line styles on a transactional email) the whole pipeline is overkill; a multi-armed bandit converges faster and more reliably. The generate→check→judge→select→rotate machine pays off only when the variant space is too large to explore by bandit.
...
Generate broadly, gate cheaply, judge strictly, select on outcomes, rotate continuously — and never optimise the generator against raw CTR. The copy that performs is not the cleverest sentence the model can write; it is the one that passed the checks, the judge, and the reward model's comparison before it cost a single send.
