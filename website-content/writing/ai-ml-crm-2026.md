...
The cap itself hides the sharpest orchestration decision. On one large marketplace, a substantial minority of users turned out to be **over-subscribed** — more eligible campaigns than their daily cap — and that minority drives the large majority of active-buyer value `[CONFIRM: internal split]`. Fill their scarce slots at random, as the pre-ML version did, and the system routinely drops the relevant message and delivers an irrelevant one: the conversion lost and the unsubscribe earned in one stroke. The fix is exactly the ranker in the stage card — score every user–message pair, schedule only the top of the list — and the production footnote is that a **gradient-boosted ranker beat both a neural AutoML baseline and random selection** on this tabular task, then lifted push-attributed conversion live `[CONFIRM: magnitudes]`. Orchestration ML doesn't need to be exotic; it needs to see all the candidates.
...
**The fifth place, still emerging: the assistant as a comms channel.** Everything above assumes the platform speaks in broadcast — a push that deep-links to a surface. The emerging alternative is a conversational assistant that *initiates*. The largest marketplaces already run early versions on both sides of the market `[CONFIRM: live initiatives]`: buyer-side, assistant-originated notifications and recommendation digests that continue a dialogue the user started — the follow-up on a shortlist built in chat, where the landing *is* the conversation and every message inherits the thread's full context; seller-side, an agent that opens the first reply to an inbound lead, fenced by opt-in, a guardrail, and human escalation. As a channel it has properties no push does — per-message context measured in a whole dialogue rather than a 40-character title, a *reply* as the engagement signal instead of a tap, and consent that reads as continuing a conversation rather than receiving a campaign. Two disciplines transfer unchanged: it must route through the same balancer and per-user budget as every other sender, or it becomes the next bypass — and it must be measured against the holdout, or it becomes the next attribution myth.
...
A production instance of the first failure, worth naming because the diagnostic is reusable. On a suggested-reply surface at one large marketplace, click-through on the suggestions climbed while the downstream deal metric didn't move — and the metric framework was built to catch exactly that divergence: track the click-rate *and* the deal metric as a pair, and when they part ways, check whether the model has drifted toward safe, universal phrasings that everyone taps and nobody converts on. Clicks are activity; the deal is value; a feature can manufacture the first without the second, and only a paired read shows it.
