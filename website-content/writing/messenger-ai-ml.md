...
The model's second job is the one that pays directly: **actuation**. Scored across the corpus, a meaningful share of conversations land in "could agree" — a substantive discussion, no visible blocker, and then the buyer simply stops replying. Not a lost negotiation; a lost attention. One production mechanism recovers them. When a chat had a real exchange (at least two messages from each side), the seller spoke last, the buyer has been silent for a day, the model reads the state as *could agree*, and the buyer hasn't already closed on another listing in the same category, the platform sends a gentle follow-up into the chat on the seller's behalf — "Hello! Any questions left? Happy to answer." The seller sees a note that it was sent for them and can switch the feature off; the copy rotates through a pool of hand-written variants so a buyer who meets it twice doesn't read it as a bot. Every condition in that list is doing work: the exchange threshold keeps it out of dead chats, the 24-hour wait keeps it polite, the category check stops it nagging a buyer who already bought elsewhere, and the model gate points it only at conversations worth saving.
